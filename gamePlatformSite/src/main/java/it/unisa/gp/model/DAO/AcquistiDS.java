...
public class AcquistiDS implements Acquisti{
	...
	public Collection<AcquistiBean> doRetrieveAllByDate(String codCliente, String first, String last, String order) throws SQLException {
		...
		String selectSQL = "SELECT * FROM " + AcquistiDS.TABLE_NAME + " WHERE CODICE_FISCALE_CLIENTE = ? AND DATA_ORA BETWEEN ? AND ?";
		...
		try {
			...
			preparedStmt = connection.prepareStatement(selectSQL);
			preparedStmt.setString(1, codCliente);
			preparedStmt.setDate(2, java.sql.Date.valueOf(first));
			preparedStmt.setDate(3, java.sql.Date.valueOf(last));
